...
// ここを直し忘れると、スタッフの招待・削除がCORSで弾かれる。
// 画面には「失敗しました」としか出ないため原因が分かりにくい箇所。
//
// 個人アカウントから組織(tsuda-cong)へ移した際の旧オリジン
// https://yida1990jw-wq.github.io は、周知が済んで誰も旧URLを開かなくなったため削除した。
const SITE_ORIGIN = 'https://tsuda-cong.github.io'
const ALLOWED_ORIGINS = new Set([SITE_ORIGIN, 'http://localhost:5174', 'http://localhost:5173'])

function corsHeaders(req: Request) {
  const origin = req.headers.get('Origin') ?? ''
  return {
    'Access-Control-Allow-Origin': ALLOWED_ORIGINS.has(origin) ? origin : SITE_ORIGIN,
